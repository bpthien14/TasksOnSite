...
const champions = [
  { id: 'yasuo', name: 'Yasuo', image: 'https://ddragon.leagueoflegends.com/cdn/8.24.1/img/champion/Yasuo.png' },
  { id: 'zed', name: 'Zed', image: 'https://ddragon.leagueoflegends.com/cdn/8.24.1/img/champion/Zed.png' },
  { id: 'leesin', name: 'Lee Sin', image: 'https://ddragon.leagueoflegends.com/cdn/8.24.1/img/champion/LeeSin.png' },
  { id: 'orianna', name: "Orianna", image: 'https://ddragon.leagueoflegends.com/cdn/8.24.1/img/champion/Orianna.png' },
  { id: 'viktor', name: 'Viktor', image: 'https://ddragon.leagueoflegends.com/cdn/8.24.1/img/champion/Viktor.png'},
];
...
const generateRandomPerformance = () => {
  return {
    ...
    position: ['TOP', 'JUNGLE', 'MID', 'ADC', 'SUPPORT'][Math.floor(Math.random() * 5)],
    champion: champions[Math.floor(Math.random() * champions.length)],
    badges: [...Array(Math.floor(Math.random() * 2))].map(() => badges[Math.floor(Math.random() * badges.length)]),
  };
};
...
export function enhancePlayer(player, index) {
  return {
    ...player,
    profileIcon: player.profileIcon || `https://avatar.iran.liara.run/public/${Math.floor(Math.random() * 40) + 1}.png`,
    tier: 'CHALLENGER',// player.tier || ['IRON', 'BRONZE', 'SILVER', 'GOLD', 'PLATINUM', 'DIAMOND', 'MASTER'][Math.floor(Math.random() * 7)],
    tierIcon: 'https://opgg-static.akamaized.net/images/medals_mini/challenger.png?image=q_auto:good,f_webp,w_48&v=1746762518',
    rank: index + 1,
    champions: player.champions || (() => {
      ...
      return shuffledChampions.slice(0, count).map(champion => champion.image);
    })(),
    ...
    badges: player.badges || [...Array(Math.floor(Math.random() * 3))].map(() => badges[Math.floor(Math.random() * badges.length)]),
  };
}
  ...
  if (!enhancedMatch.players.length) {
    ...
    enhancedMatch.players = [...Array(10)].map((_, index) => {
      ...
      return {
        ...
        name: isCurrentPlayer ? (match.playerName || 'You') : `Player ${index + 1}`,
        teamId,
        ...performanceData
      };
    });
